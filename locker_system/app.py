...
# Connect to MongoDB
client = MongoClient("mongodb+srv://adirasal2003:3PRW9xWDlRdvZNuy@clusterls.vsk8udc.mongodb.net/?retryWrites=true&w=majority&appName=ClusterLS")
...
def register():
    if request.method == 'POST':
        ...
        aadhar_file = request.files.get('aadhar_file_input')
        ...
        if aadhar_file:
            if allowed_file(aadhar_file.filename) and allowed_file_mimetype(aadhar_file):
                ...
                return 'Invalid Aadhar file format. Please upload a jpg, jpeg, or png file.'
        ...
        if pan_file:
            if allowed_file(pan_file.filename) and allowed_file_mimetype(pan_file):
                ...
                if extracted_pan_number:
                    ...
                    headers = {
                        "x-rapidapi-key": "fb15ad49b0msh95530a3168758f3p142c68jsn19890e2487f0",
        ...
        aadhar_filename = aadhar_filename or ''
        pan_filename = pan_filename or ''
        photo_filename = photo_filename or ''

        if isValid:
            db.users.insert_one(
                {'username': username, 'email': email, 'aadharID': aadhar_number, 'panID': pan_number,
                 'password': password, 'aadhar_file': aadhar_filename, 'pan_file': pan_filename,
    ...
    if request.method == 'POST':
        ...
        if captured_photo and allowed_file(captured_photo.filename):
            ...
            if results[0]:
                ...
                if liveness_passed:
                    return 'Face verified successfully! Access granted to the locker.'
                else:
